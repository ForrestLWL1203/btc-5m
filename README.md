...
- strength `>= 1.5x` and past strength `>= 0.8x`: allow entry as early as
  15s into the window
...
- If the WS deque does not cover the window open, seed it from Binance 1m
  klines REST.
- Normal entry window: remaining time in `[255s, 180s]`.
- Optional early entry: remaining time up to `285s`, for earlier persistent
  signals at `>=1.5x` current strength and `>=0.8x` past strength.
- Require:
